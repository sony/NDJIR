...
# Forward
class QueryOnVoxelHash(PythonFunction):
    """
    ...
    def __init__(self, ctx, 
        ...
        super(QueryOnVoxelHash, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        voxel_hash_feature_cuda.voxel_hash_feature(
            N,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     voxel_hash_feature_cuda.grad_query(
        #         N,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            voxel_hash_feature_cuda.grad_feature(
                N,
                grad_feature_ptr, 
...
def query_on_voxel_hash(query, feature, 
    ...
    func = QueryOnVoxelHash(ctx, G0, growth_factor, T0, L, D, min_, max_, boundary_check)
    return func(query, feature)

F.query_on_voxel_hash = query_on_voxel_hash
...
def _query_on_voxel_hash(x, 
    ...
    h = F.query_on_voxel_hash(x, feature, G0, growth_factor, T0, L, D, min_, max_)
    return h

PF.query_on_voxel_hash = _query_on_voxel_hash
...
class QueryOnVoxelHashGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        voxel_hash_feature_cuda.grad_query(N, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     G0, growth_factor, T0, L, D, 
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            voxel_hash_feature_cuda.grad_query_grad_grad_output(N, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
        ...
        #                                             self._boundary_check, accum[1])
        if propagate_down[2]:
            voxel_hash_feature_cuda.grad_query_grad_feature(N, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      G0, growth_factor, T0, L, D, 
...
def grad_query(grad_output, query, feature, 
               ...
               min_=[-1, -1, -1], max_=[1, 1, 1],
               boundary_check=False, ctx=None):
    func = QueryOnVoxelHashGradQuery(ctx, G0, growth_factor, T0, L, D, 
                                     min_, max_, boundary_check)
    return func(grad_output, query, feature)


# class QueryOnVoxelHashGradFeature(PythonFunction):
#     """
#     Gradient wrt query of QueryOnVoxelHash
...
#         super(QueryOnVoxelHashGradFeature, self).__init__(ctx)
...
#     func = QueryOnVoxelHashGradFeature(ctx, min_, max_, boundary_check, grid_sizes)
...
nnabla.backward_functions.register("QueryOnVoxelHash", voxel_hash_feature_backward)
