...
import lanczos_triplane_feature_cuda
import nnabla as nn
...
class LanczosQueryOnTriplane(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        lanczos_triplane_feature_cuda.query_on_triplane(
            B * D * 3,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     lanczos_triplane_feature_cuda.grad_query(
        #         B * D * 3,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            lanczos_triplane_feature_cuda.grad_feature(
                B * D * 3,
                grad_feature_ptr, 
...
F.lanczos_query_on_triplane = query_on_triplane
...
def _query_on_triplane(x, G, feature_size,
    ...
    h = F.lanczos_query_on_triplane(x, feature, min_, max_, use_ste)
    return h

PF.lanczos_query_on_triplane = _query_on_triplane
...
class QueryOnTriplaneGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        lanczos_triplane_feature_cuda.grad_query(B * D * 3, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     G, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            lanczos_triplane_feature_cuda.grad_query_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     lanczos_triplane_feature_cuda.grad_query_grad_query(B * D * 3, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             G, D,
        ...
        if propagate_down[2]:
            lanczos_triplane_feature_cuda.grad_query_grad_feature(B * D * 3, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      G, D,
...
class QueryOnTriplaneGradFeature(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        lanczos_triplane_feature_cuda.grad_feature(B * D * 3, grad_feature_ptr,
                                       grad_output_ptr, query_ptr, 
                                       self._G, D,
                                       self._min, self._max,
                                       self._boundary_check, False)
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            lanczos_triplane_feature_cuda.grad_feature_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                            ...
                                                            self._boundary_check, accum[0])
        if propagate_down[1]:
            lanczos_triplane_feature_cuda.grad_feature_grad_query(B * D * 3, grad_query_ptr,
                                                      grad_grad_feature_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      self._G, D,
...
nnabla.backward_functions.register("LanczosQueryOnTriplane", query_on_triplane_backward)
