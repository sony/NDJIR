...
def query_on_triline(query, triline_feature, min_, max_, window_size=2):
    B = query.shape[0]
    ...
    def interpolate(l, u, u0):
        ...
        for i in range(-w + 1, w + 1):
            ...
            fi = triline_feature[idx_l, ui]
            f = f + fi * ci

        return f
   
    fxy = interpolate(0, x, x0)
    fyz = interpolate(1, y, y0)
    fzx = interpolate(2, z, z0)
