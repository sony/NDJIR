...
import cosine_triplane_feature_cuda
import nnabla as nn
...
# Forward
class CosineQueryOnTriplane(PythonFunction):
    """
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(CosineQueryOnTriplane, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triplane_feature_cuda.query_on_triplane(
            B * D * 3,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     cosine_triplane_feature_cuda.grad_query(
        #         B * D * 3,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            cosine_triplane_feature_cuda.grad_feature(
                B * D * 3,
                grad_feature_ptr, 
...
def query_on_triplane(query, feature, min_, max_, use_ste=False, boundary_check=False, ctx=None):
    func = CosineQueryOnTriplane(ctx, min_, max_, use_ste, boundary_check)
    return func(query, feature)

F.cosine_query_on_triplane = query_on_triplane

@parametric_function_api("triplane_feature", [
    ('F', 'Grid faeture', '[3, G, G, D]', True),
])
def _query_on_triplane(x, G, feature_size,
    ...
    h = F.cosine_query_on_triplane(x, feature, min_, max_, use_ste)
    return h

PF.cosine_query_on_triplane = _query_on_triplane
...
class QueryOnTriplaneGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triplane_feature_cuda.grad_query(B * D * 3, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     G, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            cosine_triplane_feature_cuda.grad_query_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     cosine_triplane_feature_cuda.grad_query_grad_query(B * D * 3, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             G, D,
        ...
        if propagate_down[2]:
            cosine_triplane_feature_cuda.grad_query_grad_feature(B * D * 3, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      G, D,
...
class QueryOnTriplaneGradFeature(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triplane_feature_cuda.grad_feature(B * D * 3, grad_feature_ptr,
                                       grad_output_ptr, query_ptr, 
                                       self._G, D,
                                       self._min, self._max,
                                       self._boundary_check, False)
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            cosine_triplane_feature_cuda.grad_feature_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                            ...
                                                            self._boundary_check, accum[0])
        if propagate_down[1]:
            cosine_triplane_feature_cuda.grad_feature_grad_query(B * D * 3, grad_query_ptr,
                                                      grad_grad_feature_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      self._G, D,
...
def query_on_triplane_backward(inputs, min_=[-1, -1, -1], max_=[1, 1, 1], use_ste=False, 
                                  boundary_check=False):
    grad_output = inputs[0]
    query = inputs[1]
    feature = inputs[2]
...
nnabla.backward_functions.register("CosineQueryOnTriplane", query_on_triplane_backward)
