...
import lanczos_voxel_feature_cuda
import nnabla as nn
...
# Forward
class LanczosQueryOnVoxel(PythonFunction):
    """
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(LanczosQueryOnVoxel, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        lanczos_voxel_feature_cuda.query_on_voxel(
            B * D,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     lanczos_voxel_feature_cuda.grad_query(
        #         B * D,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            lanczos_voxel_feature_cuda.grad_feature(
                B * D,
                grad_feature_ptr, 
...
F.lanczos_query_on_voxel = query_on_voxel
...
def _query_on_voxel(x, grid_sizes, feature_size,
    ...
    h = F.lanczos_query_on_voxel(x, feature, min_, max_, use_ste)
    return h

PF.lanczos_query_on_voxel = _query_on_voxel
...
class QueryOnVoxelGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        lanczos_voxel_feature_cuda.grad_query(B * D, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     grid_sizes, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            lanczos_voxel_feature_cuda.grad_query_grad_grad_output(B * D, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     lanczos_voxel_feature_cuda.grad_query_grad_query(B * D, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             grid_sizes, D,
        ...
        #                                             self._boundary_check, accum[1])
        if propagate_down[2]:
            lanczos_voxel_feature_cuda.grad_query_grad_feature(B * D, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      grid_sizes, D,
...
# class LanczosQueryOnVoxelGradFeature(PythonFunction):
#     """
#     Gradient wrt query of QueryOnVoxel
...
#         super(LanczosQueryOnVoxelGradFeature, self).__init__(ctx)
...
#         lanczos_voxel_feature_cuda.grad_feature(B * D, grad_feature_ptr,
#                                        grad_output_ptr, query_ptr, 
#                                        self._grid_sizes, D,
#                                        self._min, self._max,
#                                        self._boundary_check, False)
...
#         if propagate_down[0]:
#             lanczos_voxel_feature_cuda.grad_feature_grad_grad_output(B * D, grad_grad_output_ptr,
...
#                                                             self._boundary_check, accum[0])
#         if propagate_down[1]:
#             lanczos_voxel_feature_cuda.grad_feature_grad_query(B * D, grad_query_ptr,
#                                                       grad_grad_feature_ptr,
#                                                       grad_output_ptr,
#                                                       query_ptr, 
#                                                       self._grid_sizes, D,
...
#     func = LanczosQueryOnVoxelGradFeature(ctx, min_, max_, boundary_check, grid_sizes)
...
nnabla.backward_functions.register("LanczosQueryOnVoxel", query_on_voxel_backward)
