...
import cosine_voxel_feature_cuda
import nnabla as nn
...
# Forward
class CosineQueryOnVoxel(PythonFunction):
    """
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(CosineQueryOnVoxel, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_voxel_feature_cuda.query_on_voxel(
            B * D,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     cosine_voxel_feature_cuda.grad_query(
        #         B * D,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            cosine_voxel_feature_cuda.grad_feature(
                B * D,
                grad_feature_ptr, 
...
def query_on_voxel(query, feature, min_, max_, use_ste=False, boundary_check=False, ctx=None):
    func = CosineQueryOnVoxel(ctx, min_, max_, use_ste, boundary_check)
    return func(query, feature)

F.cosine_query_on_voxel = query_on_voxel
...
def _query_on_voxel(x, grid_sizes, feature_size,
    ...
    h = F.cosine_query_on_voxel(x, feature, min_, max_, use_ste)
    return h

PF.cosine_query_on_voxel = _query_on_voxel
...
class QueryOnVoxelGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_voxel_feature_cuda.grad_query(B * D, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     grid_sizes, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            cosine_voxel_feature_cuda.grad_query_grad_grad_output(B * D, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     cosine_voxel_feature_cuda.grad_query_grad_query(B * D, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             grid_sizes, D,
        ...
        #                                             self._boundary_check, accum[1])
        if propagate_down[2]:
            cosine_voxel_feature_cuda.grad_query_grad_feature(B * D, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      grid_sizes, D,
...
#         cosine_voxel_feature_cuda.grad_feature(B * D, grad_feature_ptr,
#                                        grad_output_ptr, query_ptr, 
#                                        self._grid_sizes, D,
#                                        self._min, self._max,
#                                        self._boundary_check, False)
...
#         if propagate_down[0]:
#             cosine_voxel_feature_cuda.grad_feature_grad_grad_output(B * D, grad_grad_output_ptr,
...
#                                                             self._boundary_check, accum[0])
#         if propagate_down[1]:
#             cosine_voxel_feature_cuda.grad_feature_grad_query(B * D, grad_query_ptr,
#                                                       grad_grad_feature_ptr,
#                                                       grad_output_ptr,
#                                                       query_ptr, 
#                                                       self._grid_sizes, D,
...
nnabla.backward_functions.register("CosineQueryOnVoxel", query_on_voxel_backward)
