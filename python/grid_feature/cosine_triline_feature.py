...
import cosine_triline_feature_cuda
import nnabla as nn
...
# Forward
class CosineQueryOnTriline(PythonFunction):
    """
    Given a query (B, 3) and grid feature (3, G, D), 
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(CosineQueryOnTriline, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triline_feature_cuda.query_on_triline(
            B * D * 3,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     cosine_triline_feature_cuda.grad_query(
        #         B * D * 3,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            cosine_triline_feature_cuda.grad_feature(
                B * D * 3,
                grad_feature_ptr, 
...
F.cosine_query_on_triline = query_on_triline

@parametric_function_api("triline_feature", [
    ('F', 'Grid faeture', '[3, G, D]', True),
])
...
PF.cosine_query_on_triline = _query_on_triline


# Backward
class QueryOnTrilineGradQuery(PythonFunction):
    """
    Gradient wrt query of QueryOnTriline

    Given inputs of grad_output (B, 3), query (B, 3), feature (3, G, D),
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triline_feature_cuda.grad_query(B * D * 3, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     G, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            cosine_triline_feature_cuda.grad_query_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     cosine_triline_feature_cuda.grad_query_grad_query(B * D * 3, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             G, D,
        ...
        if propagate_down[2]:
            cosine_triline_feature_cuda.grad_query_grad_feature(B * D * 3, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      G, D,
...
def grad_query(grad_output, query, feature, 
    ...
    func = QueryOnTrilineGradQuery(ctx, min_, max_, boundary_check)
...
class QueryOnTrilineGradFeature(PythonFunction):
    ...
    Given inputs of grad_output (B, 3), query (B, 3), feature (3, G, D),
    the function outupts grad_query (3, G, D).
    """
    ...
    def forward_impl(self, inputs, outputs):
        ...
        cosine_triline_feature_cuda.grad_feature(B * D * 3, grad_feature_ptr,
                                       grad_output_ptr, query_ptr, 
                                       self._G, D,
                                       self._min, self._max,
                                       self._boundary_check, False)
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            cosine_triline_feature_cuda.grad_feature_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                            ...
                                                            self._boundary_check, accum[0])
        if propagate_down[1]:
            cosine_triline_feature_cuda.grad_feature_grad_query(B * D * 3, grad_query_ptr,
                                                      grad_grad_feature_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      self._G, D,
...
def grad_feature(grad_output, query, min_=[-1, -1, -1], max_=[1, 1, 1],
    ...
    func = QueryOnTrilineGradFeature(ctx, min_, max_, boundary_check, G)
...
nnabla.backward_functions.register("CosineQueryOnTriline", query_on_triline_backward)
