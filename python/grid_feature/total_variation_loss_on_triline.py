...
import numpy as np
import total_variation_loss_on_triline_cuda
from nnabla.function import PythonFunction


# Forward
class TVLossOnTriline(PythonFunction):
    """
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(TVLossOnTriline, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        total_variation_loss_on_triline_cuda.tv_loss_on_triline(
            B * D * 3,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[1]:
            total_variation_loss_on_triline_cuda.tv_loss_on_triline_backward(
                B * D * 3,
                grad_feature_ptr, 
...
def tv_loss_on_triline(query, feature, min_=[-1, -1, -1], max_=[1, 1, 1], sym_backward=False, 
                       boundary_check=False, ctx=None):
    func = TVLossOnTriline(ctx, min_, max_, sym_backward, boundary_check)
    return func(query, feature)


F.tv_loss_on_triline = tv_loss_on_triline
