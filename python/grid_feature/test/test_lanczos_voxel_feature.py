...
import numpy as np
import pytest
from grid_feature.lanczos_voxel_feature_composite import \
    query_on_voxel as query_on_voxel_composite
...
def test_query_on_voxel_forward_backward(seed, batch_size, grid_size, feature_size, m, M):
    nn.clear_parameters()
    ...
    feature0 = nn.parameter.get_parameter_or_create("F0", (G, G, G, D),
                                                          initializer_data0)
    ...
    feature1 = nn.parameter.get_parameter_or_create("F1", (G, G, G, D),
                                                          initializer_data1)
    output1 = F.lanczos_query_on_voxel(query1, feature1, [m] * 3, [M] * 3)
    ...
    np.testing.assert_allclose(feature0.g, feature1.g, atol=1e-5, rtol=1e-5)
...
def test_query_on_voxel_double_backward(seed, batch_size, grid_size, feature_size, m, M):
    ...
    # for g0, g1 in zip(feature0.g.flatten(), feature1.g.flatten()):
    #     print(g0, g1)
    np.testing.assert_allclose(feature0.g, feature1.g, atol=5e-3, rtol=1e-1)
