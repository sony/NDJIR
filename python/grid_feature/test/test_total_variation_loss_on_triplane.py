...
import numpy as np
import pytest
from grid_feature.total_variation_loss_on_triplane_composite import \
    tv_loss_on_triplane as tv_loss_on_triplane_composite
...
def test_tv_loss_on_triplane_forward_backward(seed, batch_size, grid_size, feature_size, m, M, sym_backward):
    nn.clear_parameters()
    ...
    feature0 = nn.parameter.get_parameter_or_create("F0", (3, G, G, D),
                                                          initializer_data0)
    output0 = tv_loss_on_triplane_composite(query0, feature0, m, M, sym_backward)
    ...
    feature1 = nn.parameter.get_parameter_or_create("F1", (3, G, G, D),
                                                          initializer_data1)
    output1 = F.tv_loss_on_triplane(query1, feature1, [m] * 3, [M] * 3, sym_backward=sym_backward)
