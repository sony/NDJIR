...
import numpy as np
import pytest
from grid_feature.lanczos_triline_feature_composite import \
    query_on_triline as query_on_triline_composite
...
def test_query_on_triline_forward_backward(seed, batch_size, grid_size, feature_size, m, M):
    ...
    feature0 = nn.parameter.get_parameter_or_create("F0", (3, G, D),
                                                          initializer_data0)
    output0 = query_on_triline_composite(query0, feature0, m, M)
    ...
    feature1 = nn.parameter.get_parameter_or_create("F1", (3, G, D),
                                                          initializer_data1)
    output1 = F.lanczos_query_on_triline(query1, feature1, [m] * 3, [M] * 3)
    ...
    np.testing.assert_allclose(feature0.g, feature1.g, atol=5e-6)
...
def test_query_on_triline_double_backward(seed, batch_size, grid_size, feature_size, m, M):
    nn.clear_parameters()
    ...
    np.testing.assert_allclose(ograd0.g, ograd1.g, atol=5e-6)
    # # np.testing.assert_allclose(query0.g, query1.g, atol=1e-6)
    np.testing.assert_allclose(feature0.g, feature1.g, atol=5e-6)
