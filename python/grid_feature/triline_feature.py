...
import numpy as np
import triline_feature_cuda
from nnabla.function import PythonFunction
from nnabla.parametric_functions import parametric_function_api


# Forward
class QueryOnTriline(PythonFunction):
    """
    ...
    def __init__(self, ctx, min_=[-1, -1, -1], max_=[1, 1, 1],
        ...
        super(QueryOnTriline, self).__init__(ctx)
    ...
    def forward_impl(self, inputs, outputs):
        ...
        triline_feature_cuda.query_on_triline(
            B * D * 3,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        # if propagate_down[0]:
        #     triline_feature_cuda.grad_query(
        #         B * D * 3,
        #         grad_query_ptr, 
        ...
        if propagate_down[1]:
            triline_feature_cuda.grad_feature(
                B * D * 3,
                grad_feature_ptr, 
...
def query_on_triline(query, feature, min_, max_, use_ste=False, boundary_check=False, ctx=None):
    func = QueryOnTriline(ctx, min_, max_, use_ste, boundary_check)
    return func(query, feature)

F.query_on_triline = query_on_triline
...
def _query_on_triline(x, G, feature_size,
    ...
    h = F.query_on_triline(x, feature, min_, max_, use_ste)
    return h

PF.query_on_triline = _query_on_triline
...
class QueryOnTrilineGradQuery(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        triline_feature_cuda.grad_query(B * D * 3, grad_query_ptr,
                                     grad_output_ptr, query_ptr, feature_ptr,
                                     G, D,
                                     self._min, self._max,
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            triline_feature_cuda.grad_query_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                          grad_grad_query_ptr,
                                                          ...
                                                          self._boundary_check, accum[0])
        # if propagate_down[1]:
        #     triline_feature_cuda.grad_query_grad_query(B * D * 3, grad_query_ptr,
        #                                             grad_grad_query_ptr,
        #                                             grad_output_ptr,
        #                                             query_ptr, feature_ptr,
        #                                             G, D,
        ...
        if propagate_down[2]:
            triline_feature_cuda.grad_query_grad_feature(B * D * 3, grad_feature_ptr, 
                                                      grad_grad_query_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      G, D,
...
class QueryOnTrilineGradFeature(PythonFunction):
    ...
    def forward_impl(self, inputs, outputs):
        ...
        triline_feature_cuda.grad_feature(B * D * 3, grad_feature_ptr,
                                       grad_output_ptr, query_ptr, 
                                       self._G, D,
                                       self._min, self._max,
                                       self._boundary_check, False)
    ...
    def backward_impl(self, inputs, outputs, propagate_down, accum):
        ...
        if propagate_down[0]:
            triline_feature_cuda.grad_feature_grad_grad_output(B * D * 3, grad_grad_output_ptr,
                                                            ...
                                                            self._boundary_check, accum[0])
        if propagate_down[1]:
            triline_feature_cuda.grad_feature_grad_query(B * D * 3, grad_query_ptr,
                                                      grad_grad_feature_ptr,
                                                      grad_output_ptr,
                                                      query_ptr, 
                                                      self._G, D,
...
nnabla.backward_functions.register("QueryOnTriline", query_on_triline_backward)
