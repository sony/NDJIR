...
void kernel_grad_query(int N, float *grad_query, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto compute_grad = [&](float scale, float gc, float c0, float c1, float d0, float d1) {
      return ograd * scale * gc * (c0 * d0 + c1 * d1);
    };

    auto scale = select_scales(i, scales);
    auto gcoef = select_grad_coefficients(i, gpqr0);
    auto su = scale.x, sv = scale.y;
    auto gcu = gcoef.x, gcv = gcoef.y;

    auto gu = compute_grad(su, gcu, b0, b1, (f10 - f00), (f11 - f01));
    auto gv = compute_grad(sv, gcv, a0, a1, (f01 - f00), (f11 - f10));
...
void kernel_grad_query_grad_grad_output(int N, float *grad_grad_output, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    // gradients
    auto compute_grad_term = [&](float ggt, float scale, float gc, float c0, float c1, float d0, float d1) {
      return ggt * scale * gc * (c0 * d0 + c1 * d1);
    };
    ...
    auto ggo_u = compute_grad_term(ggu, su, gcu, b0, b1, (f10 - f00), (f11 - f01));
    auto ggo_v = compute_grad_term(ggv, sv, gcv, a0, a1, (f01 - f00), (f11 - f10));
...
void kernel_grad_query_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto compute_grad = [&](const uint u, const uint v,
                            const float a, const float b) {
      auto f_idx = (u * stride_u) + (v * stride_v) + d;
      atomicAdd(grad_feature_i + f_idx, ograd * (ggu* su * gcu * a + ggv * sv * gcv * b));
    };
...
PYBIND11_MODULE(cosine_triplane_feature_cuda, m) {
