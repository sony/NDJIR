...
void kernel_hash_index(int N, float *output, const float *query, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) { // N = B
    ...
    // hash index
    auto idx000 = hash(x0, y0, z0, T);
    auto idx001 = hash(x0, y0, z1, T);
    auto idx010 = hash(x0, y1, z0, T);
    auto idx011 = hash(x0, y1, z1, T);
    auto idx100 = hash(x1, y0, z0, T);
    auto idx101 = hash(x1, y0, z1, T);
    auto idx110 = hash(x1, y1, z0, T);
    auto idx111 = hash(x1, y1, z1, T);
...
void kernel_voxel_hash_feature(int N, float *output, const float *query, const float *feature,
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) { // N = L * (B * R * N)
    ...
    for (int d = 0; d < D; ++d) {
      auto f000 = feature_l[feature_index(x0, y0, z0, d)];
      auto f001 = feature_l[feature_index(x0, y0, z1, d)];
      auto f010 = feature_l[feature_index(x0, y1, z0, d)];
      auto f011 = feature_l[feature_index(x0, y1, z1, d)];
      auto f100 = feature_l[feature_index(x1, y0, z0, d)];
      auto f101 = feature_l[feature_index(x1, y0, z1, d)];
      auto f110 = feature_l[feature_index(x1, y1, z0, d)];
      auto f111 = feature_l[feature_index(x1, y1, z1, d)];
...
void kernel_grad_query(int N, float *grad_query, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) { // N = L * (B * R * N)
    ...
    auto compute_grad = [&](float ograd, float scale, float a0, float a1, float b0, float b1, 
    ...
    for (int d = 0; d < D; ++d) {
      ...
      auto gx = compute_grad(ograd, sx, q0, q1, r0, r1, 
                            (f100 - f000), 
                            (f101 - f001), 
                            (f110 - f010), 
                            (f111 - f011));
      auto gy = compute_grad(ograd, sy, p0, p1, r0, r1, 
                            (f010 - f000), 
                            (f011 - f001), 
                            (f110 - f100), 
                            (f111 - f101));
      auto gz = compute_grad(ograd, sz, p0, p1, q0, q1, 
                            (f001 - f000), 
                            (f011 - f010), 
                            (f101 - f100), 
                            (f111 - f110));
      //TODO: can we reduce the calls of atomicAdd to 2 times?
...
void kernel_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) { // N = L * (B * R * N)
    ...
    for (int d = 0; d < D; ++d) {
      auto o_idx = d * (L * B) + l * (B) + b;
      auto ograd = grad_output[o_idx];
      compute_grad(ograd, x0, y0, z0, d, p0, q0, r0);
      compute_grad(ograd, x0, y0, z1, d, p0, q0, r1);
      compute_grad(ograd, x0, y1, z0, d, p0, q1, r0);
      compute_grad(ograd, x0, y1, z1, d, p0, q1, r1);
      compute_grad(ograd, x1, y0, z0, d, p1, q0, r0);
      compute_grad(ograd, x1, y0, z1, d, p1, q0, r1);
      compute_grad(ograd, x1, y1, z0, d, p1, q1, r0);
      compute_grad(ograd, x1, y1, z1, d, p1, q1, r1);
    }
  }
}
...
void kernel_grad_query_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) { // N = L * (B * R * N)
    ...
    for (int d = 0; d < D; ++d) {
      ...
      compute_grad(ograd, x0, y0, z0, d, -q0 * r0, -p0 * r0, -p0 * q0);
      compute_grad(ograd, x0, y0, z1, d, -q0 * r1, -p0 * r1, +p0 * q0);
      compute_grad(ograd, x0, y1, z0, d, -q1 * r0, +p0 * r0, -p0 * q1);
      compute_grad(ograd, x0, y1, z1, d, -q1 * r1, +p0 * r1, +p0 * q1);
      compute_grad(ograd, x1, y0, z0, d, +q0 * r0, -p1 * r0, -p1 * q0);
      compute_grad(ograd, x1, y0, z1, d, +q0 * r1, -p1 * r1, +p1 * q0);
      compute_grad(ograd, x1, y1, z0, d, +q1 * r0, +p1 * r0, -p1 * q1);
      compute_grad(ograd, x1, y1, z1, d, +q1 * r1, +p1 * r1, +p1 * q1);
    }
  }
...
PYBIND11_MODULE(voxel_hash_feature_cuda, m) {
