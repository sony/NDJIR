...
void kernel_grad_query(int N, float *grad_query, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto compute_grad = [&](float scale, float gc, float d) {
      return ograd * scale * gc * d;
    };
    auto su = select_scale(i, scales);
    auto gcu = select_grad_coefficient(i, gpqr0);
    auto gu = compute_grad(su, gcu, (f1 - f0));
    atomicAdd(grad_query + b * 3 + i, gu);
  }
}
...
void kernel_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto compute_grad = [&](const uint u, const float a) {
      ...
      atomicAdd(grad_feature_i + f_idx, ograd * a);
    };
...
void kernel_grad_query_grad_grad_output(int N, float *grad_grad_output, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    // gradients
    auto compute_grad_term = [&](float ggt, float scale, float gc, float d) {
      return ggt * scale * gc * d;
    };
    auto gg_xyz = *(float3*)(grad_grad_query + b * 3);
    auto ggu = select_ggu(i, gg_xyz);
    auto su = select_scale(i, scales);
    auto gcu = select_grad_coefficient(i, gpqr0);
    
    auto ggo = compute_grad_term(ggu, su, gcu, (f1 - f0));
...
void kernel_grad_query_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto ograd = grad_output[n];
    auto grad_feature_i = grad_feature + i * (G * D);
    auto compute_grad = [&](const uint u, const float su) {
      auto f_idx = (u * stride_u) + d;
      atomicAdd(grad_feature_i + f_idx, ograd * ggu * su * gcu);
    };
...
PYBIND11_MODULE(cosine_triline_feature_cuda, m) {
