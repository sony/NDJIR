    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        ...
        for (auto k = -w + 1; k <= w; ++k) {
          ...
          auto f_ijk = feature[feature_index(xi, yj, zk)];
          auto c_ijk = ci * cj * ck;
          f += c_ijk * f_ijk;
        }
      }
...
template<int window_size = 2>
__global__
void kernel_grad_query(int N, float* __restrict__ grad_query, 
                       const float* __restrict__ grad_output, 
                       const float* __restrict__ query, const float* __restrict__ feature,
                       int3 grid_sizes, int D, 
                       float3 min, float3 max, 
                       bool boundary_check) {
  auto Gy0 = grid_sizes.y;
    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        ...
        for (auto k = -w + 1; k <= w; ++k) {
          ...
          compute_grad(xi, yj, zk, ci, cj, ck);
        }
      }
    }
  }
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {
    ...
    auto compute_grad = [&](const uint x, const uint y, const uint z, 
      ...
      atomicAdd(grad_feature + f_idx, 
                ograd * (ggq.x * sx * a + ggq.y * sy * b + ggq.z * sz * c));
    };
    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        ...
        for (auto k = -w + 1; k <= w; ++k) {
          ...
          compute_grad(xi, yj, zk, gci * cj * ck, ci * gcj * ck, ci * cj * gck);
        }
      }
    }
  }
...
PYBIND11_MODULE(lanczos_voxel_feature_cuda, m) {
  m.doc() = "Interpolation by query on grid";
  // forward
  m.def("query_on_voxel", &voxel_feature::query_on_voxel, "");
  // 1st-order gradient
