...
void kernel_query_on_triplane(int N, float *output, const float *query, const float *feature,
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto locs = select_locations(l, xyz);
    auto u = locs.x, v = locs.y;

    // discrete points
    auto xyz0 = floorf(xyz);
    auto locs0 = select_locations(l, xyz0);
    auto u0 = locs0.x, v0 = locs0.y;
    ...
    auto feature_l = feature + l * (G * G * D);
    auto f = 0.f;
    auto w = window_size;
    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        auto vj = clamp(v0 + j, 0.f, G1);
        auto cj = lanczos(v - vj, w);

        auto f_ij = feature_l[feature_index(ui, vj)];
        auto c_ij = ci * cj;
        f += c_ij * f_ij;
      }
    }
...
void kernel_grad_query(int N, float *grad_query, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto locs = select_locations(l, xyz);
    auto u = locs.x, v = locs.y;
    auto scales_uv = select_scales(l, scales);
    auto su = scales_uv.x, sv = scales_uv.y;
    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        auto vj = clamp(v0 + j, 0.f, G1);
        auto cj = lanczos(v - vj, w);
        auto gcj = grad_coefficient(v - vj, w);
...
void kernel_grad_query_grad_grad_output(int N, float *grad_grad_output, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto gguv = select_gguv(l, ggq);
    auto ggo = gguv.x * gu + gguv.y * gv;
...
void kernel_grad_query_grad_feature(int N, float *grad_feature, 
  ...
  NBLA_CUDA_KERNEL_LOOP(n, N) {  // N = (B * R * N) * (D * 3)
    ...
    auto compute_grad = [&](const uint u, const uint v,
                            const float a, const float b) {
      auto f_idx = (u * stride_u) + (v * stride_v) + d;
      atomicAdd(grad_feature_l + f_idx, ograd * (ggu* su * a + ggv * sv * b));
    };
    ...
    for (auto i = -w + 1; i <= w; ++i) {
      ...
      for (auto j = -w + 1; j <= w; ++j) {
        ...
        compute_grad(ui, vj, gci * cj, ci * gcj);
      }
    }
  }
...
PYBIND11_MODULE(lanczos_triplane_feature_cuda, m) {
